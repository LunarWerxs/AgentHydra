...
/** Shared state threaded through every numbered self-test phase below: where the throwaway
 *  instance store lives, how a phase records a result, and the best-effort teardown list every
 *  phase that creates something can add to. */
...
/** Phase 1: a watcher pass runs to completion, right now. Deliberately RUNS one rather than
 *  reading how long ago the last one was: a freshly started daemon has legitimately never
 *  ticked, so the passive version reported a failure that was only ever a stopwatch reading.
 *  Running it proves the thing that matters - the pass completes over this machine's real state
 *  without throwing - and it is what the daemon does every 60 seconds anyway. */
async function checkWatcherPass(ctx: SelfTestCtx): Promise<void> {
  try {
    ...
    ctx.add('watcher-pass-completes', 'a full watch runs end to end', false, String(err))
  }
}

/** Phase 2: residency, BOTH on-disk shapes. The one that got away: a chat IMPORTED into the app
 *  is filed under the CLI id, a chat CREATED in the app is filed under the app's own id with the
 *  CLI id inside. A guard that knows only the first shape is blind to almost every chat the
 *  owner has. Returns the imported-shape session id so later phases can reuse the same fixture. */
function checkResidency(ctx: SelfTestCtx): string {
  const importedId = fakeSessionId()
  const createdId = fakeSessionId()
  writeFileSync(
    join(ctx.store, `local_${importedId}.json`),
    JSON.stringify({ cliSessionId: importedId, isArchived: false }),
  )
  writeFileSync(
    join(ctx.store, `local_${crypto.randomUUID()}.json`),
    JSON.stringify({ cliSessionId: createdId, isArchived: false }),
  )
  const foundImported = desktopChatArchiveState(importedId, [ctx.root]).found
  const createdVisible = desktopChatArchiveState(createdId, [ctx.root]).found
  ctx.add(
...
/** Phase 3: the surface-purity guard, asked about the owner's REAL chats. This is the check that
 *  would have caught the worst bug of the lot, and it can only be made against real state: the
 *  guard was asked "does this live in a desktop app?" and answered NO for 1,325 of 1,343 real
 *  chats, because it matched filenames and almost every real chat is filed the other way. A
 *  fixture cannot notice that; a sample of this disk can. Read-only by construction - it looks up
 *  ids, it never dispatches anything at a real chat. */
async function checkSurfacePurity(ctx: SelfTestCtx): Promise<void> {
  try {
    ...
    ctx.add('guard-sees-real-chats', 'the surface guard recognises real chats', false, String(err))
  }
}

/** Phase 4: the action gate - decide BEFORE execute, enforced. */
function checkActionGate(ctx: SelfTestCtx): void {
  ...
  try {
    ...
    ctx.add('gate-refuses-undecided', 'the action gate is enforced', false, String(err))
  }
}

/** Phase 5a: archive + unarchive round-trips the flag the app reads. */
async function checkArchiveRoundtrip(ctx: SelfTestCtx, importedId: string): Promise<void> {
  try {
    const before = desktopChatArchiveState(importedId, [ctx.root])
    const flipped = await archiveDesktopChat(importedId, true, [ctx.root], async () => false)
    const after = desktopChatArchiveState(importedId, [ctx.root])
    await archiveDesktopChat(importedId, false, [ctx.root], async () => false)
    ctx.add(
      'archive-roundtrip',
    ...
    ctx.add('archive-roundtrip', 'archiving flips the flag the app reads', false, String(err))
  }
}

/** Phase 5b: title + automation writes land on the throwaway store. */
function checkTitleAndAutomation(ctx: SelfTestCtx, importedId: string): void {
  try {
    const titled = applyDesktopChatTitle(ctx.root, importedId, 'Self-test title')
    const meta = JSON.parse(readFileSync(join(ctx.store, `local_${importedId}.json`), 'utf8'))
    ctx.add(
      'title-write',
    ...
    const stamped = applyDesktopChatAutomation(ctx.root, importedId)
    const meta2 = JSON.parse(readFileSync(join(ctx.store, `local_${importedId}.json`), 'utf8'))
    ctx.add(
      'automation-stamp',
    ...
    ctx.add('title-write', 'a chat can be given its real name', false, String(err))
  }
}

/** Phase 6: imports refuse the two things that would corrupt a thread. */
async function checkImportRefusals(ctx: SelfTestCtx): Promise<void> {
  try {
    const closed = await importSessionToDesktop({
      sessionId: fakeSessionId(),
      instanceDir: ctx.root,
      isLive: () => false,
    ...
    ctx.add('import-refuses-closed-instance', 'imports refuse unsafe targets', false, String(err))
  }
}

/** Phase 7: the messages the machinery sends still exist. */
function checkPromptsResolve(ctx: SelfTestCtx): void {
  try {
    ...
    ctx.add('prompts-resolve', 'every orchestrator message has text', false, String(err))
  }
}

/** Phase 8: the reviewer half is installed where Claude will find it. */
async function checkReviewerCommandInstalled(ctx: SelfTestCtx): Promise<void> {
  try {
    ...
    ctx.add('reviewer-command-installed', '/orchestrate is installed', false, String(err))
  }
}

/** Count `local_*.json` metadata files under one running instance's session store whose mtime is
 *  newer than the instance's own start time - on-disk changes its app may not have repainted yet.
 *  Split out of checkScreenLag so the three-level directory walk isn't rolled into that phase's
 *  own complexity. */
function countStaleMetadataFiles(instanceDir: string, startedMs: number): number {
  const dir = join(instanceDir, 'claude-code-sessions')
...
/** Phase 9: how much is stuck behind the glass right now. The honest version of "did it show
 *  up?". A metadata change written while its app was already running is on disk but not
 *  necessarily on screen: the app holds its chat list in memory and repaints at startup. That is
 *  not a theory - it is how five correctly-titled chats displayed as "General coding session",
 *  and how an archived chat kept sitting in the sidebar. Deterministic to measure: compare each
 *  metadata file's mtime against the start time of the app that owns it. */
async function checkScreenLag(ctx: SelfTestCtx): Promise<void> {
  try {
    ...
    for (const inst of running) {
      ...
      const n = countStaleMetadataFiles(inst.dir, startedMs)
    ...
    ctx.add('screen-lag', 'how much is stuck behind the glass', false, String(err))
  }
}

/** Phase 10, DEEP only: seed a real desktop chat, prove it is visible, retire it. Off by default
 *  because it leaves (and then archives) one real chat. It is the only phase that exercises the
 *  app itself, which is where every silent failure has come from. */
async function checkSeedVisibleChat(ctx: SelfTestCtx): Promise<void> {
  const seedCwd = mkdtempSync(join(tmpdir(), 'agenthydra-selftest-cwd-'))
  ctx.cleanup.push(() => rmSync(seedCwd, { recursive: true, force: true }))
  try {
    ...
    ctx.add('seed-visible-chat', 'the orchestrator can create a visible chat', false, String(err))
  }
}
...
export async function runOrchestratorSelfTest(
  ...
  const ctx: SelfTestCtx = { root, store, add, cleanup }

  try {
    await checkWatcherPass(ctx)
    const importedId = checkResidency(ctx)
    await checkSurfacePurity(ctx)
    checkActionGate(ctx)
    await checkArchiveRoundtrip(ctx, importedId)
    checkTitleAndAutomation(ctx, importedId)
    await checkImportRefusals(ctx)
    checkPromptsResolve(ctx)
    await checkReviewerCommandInstalled(ctx)
    await checkScreenLag(ctx)
    if (opts.deep) await checkSeedVisibleChat(ctx)
  } finally {
