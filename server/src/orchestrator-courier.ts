...
export function courierTaskPrompt(instanceDir: string, port: number): string {
  ...
  return [
    ...
    '',
    '4. RETIRE YOURSELF. This run left a chat in the sidebar, and the owner should never have to',
    '   look at it. As your LAST action, archive this chat:',
    `   curl -s -X POST "http://localhost:PORTNUM/api/sessions/$env:CLAUDE_CODE_SESSION_ID/desktop-archive" \\`,
    '     -H "Content-Type: application/json" -d "{}"',
    '   (a janitor sweeps any courier chat that misses this, so a failure here is untidy, not',
    '   broken - report it and carry on).',
    '',
    '5. Finish with a short recap: what you continued, what you changed, what you verified, and',
