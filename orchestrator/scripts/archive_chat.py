...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        return out({"changed": False, "report": f"REFUSED (deterministic): {err}"}, as_json, 3)
        ...
        return out({"changed": False, "report": f"{verb} FAILED: {err}"}, as_json, 1)
    ...
    # -- already there? then say so - but rule 4 does not stop applying just because the flag
    #    is already set: under a RUNNING app the chat can still be on screen, and calling that
    #    'nothing to do' is how v2's "archived" came to mean "still there".
    ...
    #    speaking again, so it lifts this for one act (never the live-writer rail).
    ...
    # -- rule 2: a live writer refuses, always, force included.
    if match.get("live"):
    ...
    # timeout above), plus the preserve POST - so 30s let a second concurrent invocation
    # decide this one had crashed and steal the lock mid-act, running the preserve/archive
    ...
    with ledgerlib.try_locked(f"archive-{session_id}", stale_secs=300) as ours:
        ...
        # -- rule 5: re-check immediately before acting. Any movement since the verdict aborts.
        try:
        ...
        # -- KNOWLEDGE PRESERVATION (owner rule, 2026-09-01): the final act before an archive is
        #    to ask the chat to update its docs. Two-phase, driven by the sweep; --force does NOT
        #    skip it (capturing knowledge is orthogonal to permission), only --no-preserve does.
        if desired and not no_preserve:
            prev = _newest_preserve(session_id)
            cur_size = _transcript_size(session_id)
            if prev is None:
                ok_sent, how = _request_preservation(session_id)
                # An undelivered request is filed as such (live smoke, 2026-09-01: it used
                # to be reported as 'asked'): later passes ask AGAIN while the grace window
                # runs, and the report says what really happened. Preservation stays
                # best-effort - a chat nothing can reach is still archived after the grace
                # window, or it would sit in the fleet forever.
            ...
            if not grew and age_min < PRESERVE_GRACE_MIN and \
                ...
                # The last request never reached the chat: ask again (best effort). When it
                # lands now, the grace window restarts from a REAL ask.
                ok_sent, how = _request_preservation(session_id)
                if ok_sent:
                    ledgerlib.note("preserve", session_id,
            ...
            # Either the turn ran (grew) or the grace elapsed with no preservation turn (a
            # dormant chat that never ran: a dead chat cannot update docs, so archive anyway
            # rather than wedge - loudly noted). THE PRESERVE ROW IS CLEARED ONLY WHEN THE
            # ARCHIVE LANDS, below (review 2026-09-01): clearing it here, before the act, meant
            # every transient act failure re-sent the whole doc-update prompt on the next pass
            # to a chat that had already written its docs - a redundant turn per retry.

        # -- act, counting the attempt BEFORE claiming anything about the outcome.
        ...
        try:
            ...
            return out(
                ...
                as_json, 1)
        if app_running:
            # RULE 4, the running-app way: never write a flag the app will re-save away - drive
            # the app's OWN control instead (immediate, durable, focus-free). Restarting is never
            # an option (owner's standing order), so this is THE path, not a fallback.
            code, ui_out = _ui_archive(str(match.get("instance")), str(title), unarchive)
            ...
            if code != 0:
                ...
                return out(
                    ...
                    as_json, 7)
            result = {"ok": True, "via": "app-ui", "detail": last}
        else:
            # App closed: the disk flag is durable and cheaper - no UI to fight.
            try:
                result = hydralib.api_post(f"/api/sessions/{session_id}/desktop-archive", {"archived": desired})
            except hydralib.DaemonError as err:
                return out({"changed": False, "report": f"{verb} FAILED: {err} (attempt recorded)"}, as_json, 1)
        ...
        # -- verify: never claim an act landed without checking.
        try:
