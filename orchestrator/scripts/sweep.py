...
def main(argv: list[str]) -> int:
    ...
    yes = "--yes" in argv
    # THE ARMED WINDOW (owner order, 2026-09-01): the unattended machinery may not act without
    # at least some occasional instruction - a person's open window (`python orch.py arm`) or
    # --force. Disarmed: fall back to a plan-only run and say so, rather than acting.
    refused = None
    if yes:
    ...
    if lanes and yes:
        ...
        try:
            ...
            doctrine = {"exit": code, "output": out}
        except Exception as e:  # noqa: BLE001 - never let enforcement break the sweep
            doctrine = {"exit": None, "output": f"doctrine pass crashed: {e}"}
