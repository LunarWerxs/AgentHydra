...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        return out({"renamed": False, "report": f"REFUSED (deterministic): {err}"}, as_json, 3)
    except hydralib.DaemonError as err:
        return out({"renamed": False, "report": f"rename FAILED: {err}"}, as_json, 1)
    ...
    # rule 1): the unattended machinery leaves held chats alone. --force is that person
    # speaking again. Closes drill.py's --rename round trip renaming a held chat out from
    # under whoever placed the hold (drill.py's --rename goes through this script).
    ...
    if code == 7:
        return out({"renamed": False, "report": text}, as_json, 1)
    if code == 3:
