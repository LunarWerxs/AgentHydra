...
CATALOG: dict[str, dict] = {
    ...
    "dryrun": {
        ...
        "platforms": "any",
        "guards": (),
        "result": "0 every run clean and self-consistent - 2 runs failed/flapped/a policy variant changed nothing - 1 could not run",
        "availability": "available",
        "summary": "OBSERVE: run the dry loop N times and prove it works - crashes, flaps, timings, policy response.",
        "notes": "each run is a fresh subprocess on purpose: looping in one process would let the usage-survey cache and module constants carry over, making later runs faster and more identical than reality.",
    },
    "policy": {
        "kind": "observe",
        "invocation": "direct",
        "platforms": "any",
        "guards": (),
        "result": "0 fine - 2 the config on disk has problems (each named) - 3 a bad argument",
        "availability": "available",
        "summary": "OBSERVE (and set): every policy knob the toolbox has, explained, with three ways to change it.",
        "notes": "writes only state/config.json - the toolbox's own policy file. It touches no chat, no account and no fleet state, which is why it is 'observe' despite being the one script whose whole job is writing something.",
    },
    "smoke": {
        "kind": "observe",
        "invocation": "direct",
