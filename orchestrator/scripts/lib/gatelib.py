...
def same_task(a: str, b: str) -> bool:
    ...
    # words above it, so the receipt never makes one task look like two. The beacon footer is
    # appended after the receipt, so it is cut first.
    a, b = receiptlib.strip(strip_beacon(a)), receiptlib.strip(strip_beacon(b))
