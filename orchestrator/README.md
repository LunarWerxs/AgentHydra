...
| `livenesslib.py` | lib | WHAT A STOPPED TURN AMOUNTS TO: `completed` / `advanced` / `plan_only` / `blocked` / `needs_approval` / `needs_followup`, plus the run's stated next action, from the last assistant text and the tool calls in that turn. The gate attaches it to every finished or idle verdict (`liveness`), the plan's `wait-on-person` decision names it (nudge a stalled plan once; send a blocked or approval-seeking chat to a person), `orch.py loop` counts it, and `fan_out status` reports it per member (`liveness`, `nextAction`). Informational only: no lane and no archive signal reads it |
