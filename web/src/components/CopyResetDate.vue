...
// Click a Weekly cell, get its reset DATE AND TIME on the clipboard as "09/18/2026 9:59 AM". Shared
// by all three instance tables (desktop, CLI, Codex) so the column behaves the same wherever it appears.
//
...
// that silently does nothing is worse than none (see resetDateTimeNumeric for why there is no guess).
import { computed } from 'vue'
import { useI18n } from 'vue-i18n'
import { toast } from 'vue-sonner'
import type { UsageLimit } from '@/lib/api'
import { resetDateTimeNumeric } from '@/lib/usage-reset'

const props = defineProps<{ limit: UsageLimit | null | undefined }>()

const { t } = useI18n()
const date = computed(() => resetDateTimeNumeric(props.limit))
...
// the fire-and-forget version told you "Copied 09/18/2026 9:59 AM" over an empty clipboard - which is worse
